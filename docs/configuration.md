...
The watcher schedules one recursive watch per non-excluded immediate child directory of the source directory rather than a single recursive watch on the root, so excluded directories (`node_modules`, `.venv`, and similar) are never registered and content under a deliberately watched dot-directory (for example, a vault rooted at `$HOME` with content under `.notes/`) delivers its own edits. New top-level directories created after start are not watched until the server restarts; the startup log line lists the scheduled root count and names.
