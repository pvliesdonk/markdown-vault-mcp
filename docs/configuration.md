...
| `MARKDOWN_VAULT_MCP_EMBED_TIMEOUT_S` | float | `30.0` | Per-request wall-clock budget in seconds for a single embedding backend call. CPU-only or large-model workloads may need 60-120 s; raise this if batches time out. |
