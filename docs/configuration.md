...
| `MARKDOWN_VAULT_MCP_OKF_VERIFY` | string | `elicit` | No | How the [`okf_verify`](tools/index.md#okf_verify) tool confirms a human review (meaningful only when `OKF_WRITE` is on, which gates the tool). With `elicit` (the default), the tool issues an MCP elicitation asking the human to confirm the review and writes the `verified` entry only on an affirmative reply, failing closed (a tool error, nothing written) when the client cannot elicit or the human declines — so a model wielding the human's token cannot self-attest a note as `human-reviewed`. Use `trust-auth` to attribute to the authenticated caller with no confirmation (refuses under auth mode `none`; only safe when the sole caller is a human-driven UI, not an agent), or `off` to hide the tool so `verified` is set only by external tooling. A non-default value with `OKF_WRITE` off is a configuration error. See the [OKF guide](guides/okf.md#recording-a-human-review). |
