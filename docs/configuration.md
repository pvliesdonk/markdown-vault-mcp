...
| `MARKDOWN_VAULT_MCP_SUMMARIZE_TIMEOUT` | float (s) | `120` | Per-request wall-clock budget for a single backend call. On timeout the tool fails with an actionable message (narrow the request, or raise this) instead of running to the SDK's ~600 s default while the client abandons the request. Set it **below** your MCP client's request timeout |
