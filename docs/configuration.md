...
| `MARKDOWN_VAULT_MCP_OKF_MODE` | string | `auto` | No | [OKF (Open Knowledge Format)](https://github.com/GoogleCloudPlatform/knowledge-catalog) read semantics. `auto` follows the vault's own declaration — an `okf_version` field in the root `index.md` frontmatter — and switches on read-side annotations (note type, lifecycle status, staleness, trust tier) in `search`, `read`, `get_context`, and `stats` when it is present. `off` never applies OKF semantics; `on` forces them without a declaration. Annotations are read-only either way: this setting never changes write behavior. Note: setting `MARKDOWN_VAULT_MCP_INSTRUCTIONS` replaces the default server instructions entirely, including the OKF guidance sentence. |
