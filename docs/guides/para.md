...
**Status means two different things.** PARA overloads `status` for workflow state (`active`, `archived`), while OKF reads `status` as a lifecycle (`draft`, `stable`, `deprecated`). An archived project is not a deprecated concept, and the two vocabularies have no sensible mapping. On an OKF bundle, then, keep PARA's workflow state in its own frontmatter key (`workflow` or `para_status`) and leave `status` to OKF. The server preserves any extra keys you add, and its ranking reads only the OKF meaning, so an archived-but-current note is not quietly demoted.
