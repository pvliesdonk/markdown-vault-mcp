...
src/markdown_vault_mcp/
  ...
  utils/
    ...
    links.py           -- link target computation and replacement; decode_link_target/decode_markdown_destination read a destination, build_plain_destination/encode_plain_destination/escape_link_text write one (#1494, #1513); find_bracket_span/iter_bracket_links/iter_inline_links live here, not in scanner.py, so the index and the rewrite agree on which links exist and where each destination starts (#1521, #1526); iter_bracket_links holds CommonMark's delimiter stack for both link families, each passing only its own test for what must follow a closed span (#1528) — the reference family's is a closure over the definition table, because its ladder resolves against it (#1531); inline_link_follows is that ladder's rung zero, asking whether an inline link *closed* on a span rather than whether it yielded a target, since §6.3 gives the inline form precedence and the shortcut rung resolves on text alone
  ...
  git/
    ...
    _run.py            -- low-level git subprocess + credential plumbing; literal_pathspec (#1303)
