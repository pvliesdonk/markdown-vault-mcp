...
1. **End-to-end spike.** The three criteria from this PR's critical review were **verified locally on 2026-08-18** (knope 0.23.0, scratch repo mirroring the real manifests): (a) re-dispatch refresh = recreate-from-base + force-push updating the same PR, with the never-rerun-on-a-stale-branch rule (§4.2) — *verified*; (b) the same-source guard runs pre-tag, and a refusal provably leaves no tag behind, with the merged-prep recovery caveat now in §4.4 — *verified*; (c) the `uv.lock` self-version stamp stays inside the stamps-only promotion diff across a full rc→stable cycle, with the SemVer/PEP 440 rc-form note in D12 — *verified*. Still open, needing a scratch **GitHub** repository with the real workflows: knope's `CreatePullRequest` step, the merge-triggered `Release` workflow, and the token question — in particular whether a `GITHUB_TOKEN`-created tag/release fires the downstream `release: published` workflows (GitHub suppresses workflow triggers from `GITHUB_TOKEN`-initiated events, which is why today's pipeline uses `RELEASE_TOKEN`; the spike must confirm the same pattern carries over). Go/no-go for the fallback.
...
3. **First release under the new system** and the disposition of `v4.0.0-rc.1` / `release/4.0`. The spike narrowed this (§9.1): knope continues a series from the highest *reachable* rc tag of the target version — so preparing 4.0 on `release/4.0` (which contains the PSR-tagged commit) continues correctly at `rc.2` and promotes to `4.0.0` with no yank needed; only a trunk-prepared 4.0 series, where that tag is unreachable, would restart at `rc.0` below the existing `rc.1` and make yanking necessary. A migration choice, parked until the vision is agreed.
