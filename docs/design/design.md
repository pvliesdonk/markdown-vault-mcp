...
- `_name_segments(git_root, cur_rel, env)` (#1306): the names a note carried, newest first, each paired with the revisions where that name was the note's own — the second half of the boundary above, and the one `--follow` cannot supply. `--follow` resolves identity from a single parent's diff, so a rename performed *while resolving a merge* — which belongs to no parent's diff — leaves it reporting the merge as the note's creation, and the name before it unreachable at any similarity threshold. The names are recovered instead from the walk `get_file_at_ref` already trusts (`git log -m --first-parent --follow --name-status`, argv shared through `_first_parent_records_cmd`), so the two readers identify a note the same way. Each segment carries `after` (a commit excluded from it, and with it everything older: the commit that gave the note that name, or the parent of the commit that created it) and `through` (the commit that renamed the note away, included, because it is the last to touch that name). Those bounds are what keep the enumeration honest without a lineage filter: `after` excludes whatever held the name before this note, and a note that reuses a name *after* the rename is not an ancestor of the rename commit. The walk yields nothing — leaving both readers exactly as they were — for a note that never changed name, whose `--follow` walk already covers it, and for one whose walk never reached the note's birth, where the oldest name's boundary is unknown.
