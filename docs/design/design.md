...
- `get_file_at_ref(repo_path, path, ref)`: returns the file's text at `ref` and the vault-relative path it carried there, so the caller passes the note's *current* path. Rename resolution is `_path_at_ref_via_follow`, **not** the whole-range `resolve_path_at_ref` the diff path uses: that helper compares the blob at `ref` with the blob at `HEAD`, so a note both rewritten and renamed since falls under the similarity threshold and reads as an unrelated delete plus add — the exact shape an overwrite-then-move produces, which is the shape this has to survive. Instead it walks `git log --follow` over `ref..HEAD` and takes the oldest commit that touched the file: nothing touched the file between `ref` and it, so its name there is its name at `ref` — unless that commit is itself the rename, where `--follow` reports the *new* name and the old one comes from a single-commit `resolve_path_at_ref(git_root, f"{sha}^", …, to_ref=sha)`, the same move the per-commit diff path makes.

  Paths come back NUL-delimited (`-z`, parsed by `_parse_z_log_paths`) rather than in git's default line-oriented rendering. Git quotes and octal-escapes any path it considers unusual — non-ASCII, or holding a newline, tab, or quote — and that rendering read back as a literal filename resolves to nothing. `-z` is the whole answer rather than `core.quotePath=false`, which suppresses only the non-ASCII half; the layout is `\x1e<sha>\x00<short>\x00<iso>\x00<subject>\x00\n<path>\x00`, where the one newline after the subject's terminator is git's separator between the header and the name list and the only one the format inserts, so dropping exactly it recovers a path that contains newlines of its own. Output is decoded explicitly rather than through `text=True`, whose universal-newline translation would rewrite a CR inside a path. The two `--name-only` readers that still parse the quoted form (`get_file_history`, `_per_commit_diffs`) are tracked in #1282.

  Three failure modes are deliberate. A historical path that does not lie under the vault prefix raises **before** the blob is read: rename resolution walks the whole repository, so on a vault nested inside a larger one a note moved in from a sibling directory resolves outside the vault, and serving it would hand the caller content the vault does not govern. `git show <ref>:<historical>` is captured as bytes and decoded through `decode_utf8`, so a blob that is not valid UTF-8 raises instead of arriving mangled by replacement characters, and a leading BOM is dropped exactly as the scanner drops it before parsing a note on disk (#673) — left in place it would sit before the `---` and hide the frontmatter block. An unknown ref, or a path absent at `ref`, raises `ValueError` the same way.
