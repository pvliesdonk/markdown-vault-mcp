...
pathspec rules: [`reference/git-staging-and-commits.md`](reference/git-staging-and-commits.md) § Pathspecs.) A note may
...
sibling's uncommitted edit into the note's own commit, which is the
per-operation staging guarantee below failing through the note's name rather
than through a missing pathspec. Every command in `git/` that takes a pathspec
therefore builds it with `literal_pathspec` (`git/_run.py`), which prefixes
git's `:(literal)` magic. `check-ignore` is the one exception and not an
...
accept: [`reference/git-staging-and-commits.md`](reference/git-staging-and-commits.md) § Staging.) One MCP write operation produces
...
[`reference/git-staging-and-commits.md`](reference/git-staging-and-commits.md) § Committing and identity.) Scoping the decision left the act
...
| `MARKDOWN_VAULT_MCP_SOURCE_DIR` | Path to markdown files; the directory must exist unless managed git clones into it | required |
...
[`reference/git-push-and-remotes.md`](reference/git-push-and-remotes.md),
[`reference/git-staging-and-commits.md`](reference/git-staging-and-commits.md) and
...
`safe.directory` in [`reference/git-push-and-remotes.md`](reference/git-push-and-remotes.md) § Repository discovery and refs): all sync operations (startup unpushed
...
- `_name_segments(git_root, cur_rel, env)` (#1306): the names a note carried, newest first, each paired with the revisions where that name was the note's own — the second half of the boundary above, and the one `--follow` cannot supply. `--follow` resolves identity from a single parent's diff, so a rename performed *while resolving a merge* — which belongs to no parent's diff — leaves it reporting the merge as the note's creation, and the name before it unreachable at any similarity threshold. The names are recovered instead from the walk `get_file_at_ref` already trusts (`git log -m --first-parent --follow --name-status`, argv shared through `_first_parent_records_cmd`), so the two readers identify a note the same way. Each segment carries `acquired_at` (the commit that gave the note that name — its rename or its creation), `after` (that commit's first parent: excluded, and with it everything older) and `through` (the newest commit at which the name was the note's own). Those bounds are what keep the enumeration honest without a lineage filter: `after` excludes whatever held the name before this note, and a note that reuses a name *after* the rename is not an ancestor of the rename commit. `acquired_at` settles the one commit two segments both contain — the rename touches both names — and it settles it the same way whatever order the names were discovered in.

  A first-parent walk alone is not enough for one shape, which the review of #1314 found: where a merge-resolution rename joined a branch that had *also* renamed the note, the merge records one rename, from the trunk's name straight to the name it settled on, and the name the branch used appears in no first-parent diff at all — so the branch's own commits, made under that name, went unreported and its rename rendered as the trunk's name being deleted. Each other parent of a rename-carrying merge is therefore asked what the note was called at it (`resolve_path_at_ref` against the merge), and a third name opens a walk of its own, bounded to that branch. Discovery is capped at `_MAX_NAME_WALKS` chains: past it the recovery reports fewer of the note's names, never names that are not the note's.

  The walk yields nothing — leaving both readers exactly as they were — for a note that never changed name, whose `--follow` walk already covers it, and for a line of descent that never reached the note's birth, where the oldest name's boundary is unknown.

- The two single-note readers then **supplement** what `--follow` found, rather than replacing it (`_with_earlier_names` for history, `_rows_with_earlier_names` for the per-commit diff). One `git log --format=%H` per segment, bounded to that segment and carrying the caller's own `--since` / `--until` / `-n` (and, for the diff, `--not <ref>`), collects the commits under each earlier name; anything the caller does not already have is added. Deliberately **no `--follow` on these walks**: the names are known already, and leaving rename detection out is what lets git apply its ordinary merge simplification — so a merge is listed only where it changed the note relative to what it merged, and the commits from a merged branch stay individually listed rather than collapsing into the merge. (Passing `-m --first-parent` to the enumeration would have been the small fix and the wrong one: it substitutes the merge for the branch commits that a caller asking who changed a note wants to see.) Where something *is* added, the union is reordered by `git log --no-walk`, which puts commits gathered from several walks into one dated sequence without this module comparing timestamps across time zones; where nothing is, the caller's result is returned untouched. The per-commit walk additionally needs the name the note had at each commit, since `git show <sha> -- <today's name>` returns nothing for a commit that predates the rename — the commit that *performed* a rename is named by what the note became, which is the name `_rename_aware_diff_args` has to target for git to pair the rename rather than report a deletion.

- `get_file_at_ref(query)` (#1137): returns a note's content at a revision, resolved **by note rather than by path**. Git records paths, blobs, and add/rename events and nothing else, so identity is derived only from what it records: `git log -m --first-parent --follow --name-status -z --find-renames=30 <ref>..HEAD -- <path>` is walked newest-first carrying a tracked path. An `R old new` record whose *new* side is tracked moves the tracked path back; an `A` on the tracked path stops the walk and **raises**, because git is saying the file at that path was created within the range; a `C` (copy) raises for the same reason; `D`/`M`/`T` continue; any other record raises, since an unrecognised record is not evidence of continuity. `-m --first-parent` is load-bearing rather than decorative: a rename performed *while resolving a merge* belongs to no parent's diff, so a plain walk returns no records at all and the caller would fall through to trusting today's path at an old revision. `--find-renames=30` matches `resolve_path_at_ref`'s threshold (#338); the rename limit is pinned with `-c diff.renameLimit` so the boundary is a property of this code rather than of the operator's git config. Two checks bracket the walk: `merge-base --is-ancestor` (a revision on rebased-away history cannot anchor the range) and `ls-files` (a note on disk that git never tracked leaves no creation record to stop the walk, so the walk alone would return the *previous* file's bytes under the new note's name). The content lookup then deliberately avoids `<ref>:<path>`: that is a revision expression rather than a pathspec, and an unmatched one resolves to whatever else git can parse — for a note named `star*note.md` it returns the *commit*, after which `cat-file blob` fails on stderr with exit status **zero**, which reads back as an empty note. Instead `git ls-tree -l -z <ref> -- <pathspec>` yields the entry's mode, object id and size in one call, with the pathspec built literally like every other in this package (above); a symlink (mode `120000`, whose blob holds the link target rather than note content) or a non-blob entry is rejected there, the size is checked against `max_note_read_bytes` — except for a `section=` read, which is exempt exactly as the on-disk path is — and only then is the blob fetched by its own hash and decoded explicitly, so non-UTF-8 content is a `ValueError` rather than a decode error escaping the subprocess layer. A Git LFS pointer is refused rather than returned: the working tree holds real bytes only because the smudge filter ran at checkout, which this path does not go through, so restoring what git has here would write the pointer over the note. The path the walk resolves to must also lie beneath the vault prefix — the caller's path is confined by `validate_history_path`, but a repository can hold far more than the vault, and a note renamed in from outside carries a path from outside. Unlike its siblings this **raises** on a vault without git: an empty string is indistinguishable from "the note was empty at that revision", which a caller about to restore content must not be handed.
- `committed_revision(repo_path, path)` (#1137): the overwrite breadcrumb. Returns the note's newest commit **only when the working copy still matches it** — the note must be tracked (`ls-files`) and `git diff --quiet <that commit> -- <path>` must be clean. The comparison is against the *named commit* rather than `HEAD` deliberately: the commit worker runs on its own thread and does not hold the vault's write lock, so a HEAD-relative check can answer about a commit the first probe never named. Returns `None` for every unanswerable case rather than raising, because a write must never fail over a breadcrumb. The probe and the write are held under the vault's re-entrant write lock by `WriterFacet` (`DocumentManager.write_scope()`), because a decision made from the current state is only true if nothing else writes the note between reading that state and acting on it: without it, a concurrent rename could put a *different* note at the path and the breadcrumb would name a commit belonging to a note this write never replaced. The bulk maintenance path calls `DocumentManager.write` directly and so pays neither the lock hold nor the probe.
...
Later decisions (2026-08-14, #1035):
...
Later decision (2026-09-24, #1581, #1590):

| # | Topic | Decision | Rationale |
|-|-|-|-|
| 29 | `SOURCE_DIR` is required | The variable stays required at `from_env` (unset or blank is a `ConfigurationError`), and the directory must exist where a vault is built: `Service.start` and the CLI's `_build_vault` call `ensure_source_dir`, which raises the same error class naming the variable (managed git mode excepted, its bootstrap clones into the directory). The dataclass keeps a default of `/data/vault` only because the template's contract tests construct `ProjectConfig()` with no arguments (template #621); it is not an operator default, and the generated `Required` column reading `No` is that template limitation, tracked upstream. | A vault without a directory is an error, not a configuration. An attempt to default the variable at construction to satisfy the template's `test_model_facing_text.py` was reverted: that test bypasses the `config_contract_env` seam the template's own `test_config_contract.py` honours (template #652, reopened), and the seam exists for exactly this kind of required field. A wrong path used to crash the lifespan with a raw `FileNotFoundError` from the file watcher; the existence check fails fast with the variable named instead. |
