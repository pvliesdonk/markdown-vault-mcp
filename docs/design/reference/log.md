...
- Extended [Git history and revision queries](git-history-queries.md) for
  ...
  exits 1 from `rev-parse --verify --quiet HEAD`, and `merge-base
  --is-ancestor` exits 1 only for "not an ancestor" (128 for an unknown id). Observed on git 2.55.0, the `rev-parse`
