...
- `rebase <ref>` commits each replayed change, so it needs a committer identity like `commit` does: with none in repo, global or system config, no `GIT_COMMITTER_*`/`GIT_AUTHOR_*`, and no auto-detectable `user@host` (a container hostname yields `root@<id>.(none)`), it stops at the first replayed commit with `Committer identity unknown`, exit 128, and **no unmerged paths** — `diff --diff-filter=U` is empty, so a resolver that only knows unmerged paths has nothing to act on. `user.useConfigOnly=true` disables the auto-detection so the failure reproduces on a developer machine too; an identity given via `-c user.*` or the `GIT_*` env still counts. [source: git-commit] (`user.useConfigOnly`) [source: git-var] [observed: git 2.47.3 in the published container; git 2.55.0 with `user.useConfigOnly`] [pins: tests/test_git_force_methods.py::TestForceMethodsErrorBranches::test_force_pull_rebases_without_any_repo_or_global_identity]
