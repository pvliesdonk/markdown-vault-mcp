...
- `--author='Name <email>'` overrides the author only; the committer comes from `GIT_COMMITTER_NAME`/`GIT_COMMITTER_EMAIL` when set, else from `user.name`/`user.email` (`-c user.*` included) — the environment overrides configuration, so `-c user.*` does **not** beat `GIT_COMMITTER_*`; an `--author` not of that form is a search pattern and, matching nothing, dies `--author '<v>' is not 'Name <email>' and matches no existing author`. [source: git-commit] (`--author`, COMMIT INFORMATION) [source: src-commit] [observed: git 2.43.0, `cat-file commit`: `author AN <ae@x>` / `committer T <t@x>`; git 2.55.0, `GIT_COMMITTER_NAME=EnvC` with `-c user.name=Conf` records `committer EnvC`] [pins: tests/test_git.py::TestOidcClaimAuthorCommitterSplit::test_principal_sets_author_committer_stays_static, tests/test_git.py::TestCommitterIdentityInCommit::test_custom_committer_in_commit_flags]
...
- `rebase <ref>` commits each replayed change, so it needs a committer identity like `commit` does: with none in repo, global or system config, no `GIT_COMMITTER_*`/`GIT_AUTHOR_*`, and no auto-detectable `user@host` (a container hostname yields `root@<id>.(none)`), it stops at the first replayed commit with `Committer identity unknown`, exit 128, and **no unmerged paths** — `diff --diff-filter=U` is empty, so a resolver that only knows unmerged paths has nothing to act on. Identity is taken from `GIT_AUTHOR_*`/`GIT_COMMITTER_*`, else `user.name`/`user.email`, else `EMAIL`, else the system user name with the outgoing-mail hostname (`/etc/mailname`, falling back to the fully qualified hostname); `user.useConfigOnly=true` disables that final guess so the failure reproduces on a developer machine too, and an identity given via `-c user.*` or the `GIT_*` env still counts. [source: git-commit] (COMMIT INFORMATION) [source: git-config] (`user.useConfigOnly`) [observed: git 2.47.3 in the published container; git 2.55.0 with `user.useConfigOnly`] [pins: tests/test_git_force_methods.py::TestForceMethodsErrorBranches::test_force_pull_rebases_without_any_repo_or_global_identity]
...
- When resolving a conflict leaves the replayed commit with no change of its own — every conflicting path took upstream's side, as the reserved-file rules do for an `index.md` (#1395) — `rebase --continue` drops that commit and completes with exit status 0 ("Successfully rebased"); it does not stop asking for `--skip`. HEAD then equals the upstream tip plus any non-empty replayed commits, which is why the pipeline tells a completed rebase from one that never started by whether HEAD moved, not by whether anything was saved for a sibling. [observed: git 2.55.0]
  [pins: tests/test_git_conflict_projection.py::TestReservedFilesResolveInPlace::test_all_reserved_conflict_leaves_no_sibling_and_merges_the_log]
...
- **`_git_env` (`strategy.py`) / `git_env` (`_run.py`)** — every git subprocess of the pull pipeline (`rebase`, `rebase --continue`, the resolver's `checkout`/`add`) receives the configured commit identity as `GIT_AUTHOR_*`/`GIT_COMMITTER_*`, so a served clone with no `user.*` config rebases as the same principal a per-write commit uses (`-c user.*`); before #1362 only the askpass variables travelled and the rebase inherited whatever the process had, which in the container was nothing. Because the environment outranks `-c user.*` (the `--author` claim above), the `-c user.*` pair `write_conflict_files` still passes is redundant under this env — the values are the same either way — while `_commit_staged` runs without the env and decides its committer by `-c` alone. (#1362.)
