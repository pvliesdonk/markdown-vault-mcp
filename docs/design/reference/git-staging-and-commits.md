...
### Committing and identity (`strategy.py`: `_commit_staged`, `_sanitize_git_identity`)
...
- `git config user.email` exits 0 with the value, or 1 printing nothing when unset at every scope; committing with no identity anywhere dies `Author identity unknown`, 128. [source: git-config] (exit status, ret=1) [observed: git 2.43.0, `GIT_CONFIG_GLOBAL=/dev/null`] [pins: tests/test_git_constructor.py::test_default_committer_without_checkout_identity_needs_no_warning]
...
- **Startup identity probe removed (#1236)** — the former `_check_identity` read only `git config user.email`. The strategy already supplies its configured/default committer to writes and rebases, so missing checkout identity no longer produces a warning. The per-request unusable-claim warning remains owned by `_identity.py`.
