...
- [obsidian-git and isomorphic-git in a shared vault](/obsidian-git.md): the plugin's default commit subject and identity, `syncMethod`/`mergeStrategy` defaults, conflict behaviour on desktop (simple-git) and mobile (isomorphic-git: no rebase, `mergeDriver` only, no `.gitattributes`), and what Obsidian writes on a new note. Read before anything that classifies commits by origin or curates `log.md` from history (`okf-ownership.md` §5, §6).
