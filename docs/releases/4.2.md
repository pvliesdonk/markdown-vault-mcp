...
**A rebase conflict on `index.md` or `log.md` no longer damages the bundle.**
The pull's conflict policy treated the OKF reserved files like notes: it kept
the remote's version, saved the server's as an `index.conflict-mcp-…md`
sibling, and wrote `conflict_with` into both. On a bundle that made the
sibling a stray untyped concept listed by the next regeneration, put a key the
spec forbids on the root index, and left the server's log bullets in a file
nobody reconciles
([#1395](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1395)).
While the server maintains those files (`OKF_WRITE` on an OKF-active vault),
a conflict on them is now resolved in place: `index.md` takes the remote's
version and is re-derived on the next regeneration, `log.md` keeps both
sides' bullets, and no sibling or marker is written. A pull whose only
conflicts were reserved files reports `reason="rebased"`. Without `OKF_WRITE`
nothing changes: the files are notes and get siblings as before.
