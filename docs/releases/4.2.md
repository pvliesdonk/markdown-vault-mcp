...
**`okf_validate` now reports frontmatter an `index.md` may not carry.**
The format permits frontmatter in an index file in exactly one place, the
bundle root's `okf_version` key, but the audit read the root index's other
keys nowhere and a folder index's frontmatter not at all, so a bundle
carrying either was reported conformant
([#1396](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1396)). A
new conformance finding, `index_frontmatter`, lists the files. The keys the
vault's own `required_frontmatter` gate makes the server seed into generated
reserved files are tolerated, so a gated vault is not told to fix what the
server wrote on purpose; any other key is reported, including on a vault
with no gate, where a hand-authored `title` on the root index is the
operator's and the format does not allow it. A folder index declaring
`okf_version` appears under both this finding and `misplaced_okf_version`,
which say different things about the same file. `log.md` is untouched: the
no-frontmatter rule belongs to the index section, and the log section states
no frontmatter rule at all.
...
- **A `required_frontmatter` listing `okf_version` no longer has that key
  seeded into generated index files.** Since 4.1.0 the server seeded every
  required field into the `index.md` and `log.md` it writes, so a gate
  naming `okf_version` had the key written into each folder's index with an
  empty value, a declaration the format allows only on the bundle root
  ([#1396](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1396)).
  The server now leaves it out, so those generated index files stay
  conformant and, missing a field the gate requires, drop out of `search`
  and `list_documents` until the gate stops requiring the key.
  `okf_validate` reports each one under `misplaced_okf_version` and
  `index_frontmatter`. Only vaults naming `okf_version` in
  `required_frontmatter` are affected; on every other vault nothing
  changes. Whether this gate should reach the files the server generates at
  all is open
  ([#1438](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1438));
  if you rely on this configuration, say so there.
