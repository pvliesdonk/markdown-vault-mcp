...
**The maintained `index.md` listings now follow every change, not only a
write.** Under `OKF_WRITE` a folder's listing was regenerated only after an
MCP `write`, `edit` or `append` into that same folder. A rename, a delete, a
folder move, and anything that arrived through a git pull or the file
watcher left it stale until such a write happened to land there
([#1392](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1392)). The
listing is a projection of the index, so it is now regenerated after each of
those as well, for the folder and its ancestors, and a note written into a
new subfolder shows up in the parent's pointers at once. A regeneration that
changes nothing leaves the file alone, so the file watcher does not chase
its own writes, and the listings a pull refreshes land as one commit. A
full rebuild refreshes every listing, since it can name no smaller set: that
covers a cold start, whose first build absorbs the startup pull, and
`reindex` with `force`. The
`reindex` tool's result gains a `folders_changed` list.
