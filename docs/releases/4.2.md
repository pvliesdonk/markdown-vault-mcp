...
- **A `required_frontmatter` listing `okf_version` no longer has that key
  ...
  The server now leaves it out. Such a gate asks of every note what the
  format permits on one file, so nothing satisfies both; the generated
  index files stay conformant and, missing a field the gate requires, drop
  out of `search` and `list_documents` until the gate stops requiring the
  key. `okf_validate` reports each one under `misplaced_okf_version` and
