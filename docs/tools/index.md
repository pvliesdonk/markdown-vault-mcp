...
!!! note "The `folder` argument"
    Every tool taking a `folder` argument reads it the same way. Omitting it (or passing `null`) applies no folder restriction. Passing the empty string `""` restricts results to **root-level documents only**, since the root folder is spelled `""` in `list_folders` output and in each result's `folder` field. Any other value selects that folder and its sub-folders, and is matched after folding backslashes to forward slashes and stripping surrounding slashes, so `"Journal"`, `"Journal/"` and `"/Journal/"` all select the same notes.
