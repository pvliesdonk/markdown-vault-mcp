...
- **LLM summarization**: optional `summarize` tool condenses a note, a set of notes, or a subtree with a language model via any OpenAI-compatible endpoint (OpenAI, Ollama, Anthropic's compat endpoint, vLLM, ...); the synthesis references the individual source notes by path. Gated on `OPENAI_API_KEY` or a configured base URL.
