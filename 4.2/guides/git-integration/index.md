...
A webhook credential on an HTTP or SSE transport disables the filesystem watcher, the same way `GIT_PULL_INTERVAL_S > 0` does. Git rewrites the working tree during a checkout, and a watcher firing mid-checkout would scan a partial tree. Reindexing stays driven by the webhook and the periodic loop. See [File Watcher](https://pvliesdonk.github.io/markdown-vault-mcp/4.2/configuration/#file-watcher).
...
The variables themselves are listed under [Change detection](https://pvliesdonk.github.io/markdown-vault-mcp/4.2/configuration/#change-detection) in the configuration reference.
...
The full enumeration of `pull.reason` and `push.reason` values lives in the [`git_sync` tool reference](https://pvliesdonk.github.io/markdown-vault-mcp/4.2/tools/#git_sync).
