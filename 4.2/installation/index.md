...
The `latest` tag is the newest stable release. For early adopters who want to test unreleased changes, the rolling `edge` tag tracks every merge to `main` and carries no version identity; see [Image tags](https://pvliesdonk.github.io/markdown-vault-mcp/4.2/deployment/docker/#image-tags) for the full list. The floating `:latest`, `:vN`, and `:vN.M` tags only move on stable releases.

```
docker pull ghcr.io/pvliesdonk/markdown-vault-mcp:edge
```

See [Docker deployment](https://pvliesdonk.github.io/markdown-vault-mcp/4.2/deployment/docker/index.md) for compose setup and volume configuration.
...
See the [systemd deployment guide](https://pvliesdonk.github.io/markdown-vault-mcp/4.2/deployment/systemd/index.md) for full configuration and troubleshooting.
...
See the [Claude Code plugin guide](https://pvliesdonk.github.io/markdown-vault-mcp/4.2/guides/claude-code-plugin/index.md) for configuration and usage details.
