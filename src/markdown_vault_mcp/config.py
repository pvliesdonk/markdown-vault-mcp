...
class ProjectConfig:
    ...
    # server_name is declared by the template-owned config-presentation.yml
    # (template provenance), so it carries no metadata here and is read
    # outside from_env (read_server_name) to keep the AST scan from
    # double-declaring it. There is no instructions field: pvl-core 5's
    # finalize_instructions owns the INSTRUCTIONS / INSTRUCTIONS_EXTRA env
    # contract (#1162).
