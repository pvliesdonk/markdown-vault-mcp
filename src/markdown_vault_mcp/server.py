...
def make_server(
    *,
    ...
    # Domain server identity: attach the vault icon. FastMCP's ``icons``
    # property is read-only (only the constructor accepts icons, which the
    # skeleton body does not), so write the low-level server field it reads.
