...
A thin **wrapper** over the :class:`~markdown_vault_mcp.indexing.IndexWriteCoordinator`
(PR1), exposing only the public index operations — build / reindex /
embeddings (sync + async) plus the readiness and writer-status queries. It
deliberately does NOT expose the coordinator's internal surface (``close``,
``writer``, ``require_built``, ``mark_paths_dirty``, ``rebuild_embeddings``),
which the root owns. Part of the ``collection.py`` facade decomposition (#576).
"""
...
class IndexFacet:
    """Index build, readiness, and writer-status operations.

    Wraps the :class:`IndexWriteCoordinator`; every method delegates 1:1.
    """

    def __init__(self, coordinator: IndexWriteCoordinator) -> None:
        """Hold the coordinator the index operations delegate to.
