...
This major release moves the server to FastMCP 4 and makes a fresh Compose deployment work on its host without a reverse proxy. Logging settings change too: logs are structured and default to JSON off a terminal. Writable servers reject blind whole-file replacement by default, while Python consumers move configuration into `VaultSettings` and remove two deprecated Git identity arguments. Links the server writes stay valid in Obsidian, the link graph holds more of the links a reader follows, and large vaults no longer pay a full index scan for every changed note. Notes deleted or renamed in a pulled commit no longer linger in listings when the reindex after the pull fails. OKF no longer treats a static bearer credential as proof that a person acted. A vault directory that does not exist no longer crashes the server; it starts, and every tool names the variable to set. Operators and library consumers need to follow the migration steps in [Upgrading](#upgrading).

## FastMCP 4 and a standalone container deployment

The server moves from FastMCP 3.4.7 to 4.0.9, through `fastmcp-pvl-core` 9.0.1 and copier template 9.3.0 ([#1465](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1465), [#1550](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1550), [#1570](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1570), [#1591](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1591), [#1601](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1601), [#1587](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1587), [#1614](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1614)). Library environments that pin FastMCP 3 must move to the FastMCP 4 dependency line. The application handles the protocol changes: background tasks register with each server instance, and `okf_verify` uses an input-required response on modern sessions while retaining server-initiated confirmation for older clients ([#1271](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1271), [#1465](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1465)).

Compose no longer assumes Traefik or an existing proxy network. It pulls the published image, publishes the server on `127.0.0.1:8000`, and starts it with its state and vault volumes. Only the Docker host can connect, because the quick start runs without authentication. To serve other machines, first set a bearer token or OIDC in `.env`, then publish on every interface from `compose.override.yml` with `ports: !override` and the mapping `"8000:8000"` ([template#673](https://github.com/pvliesdonk/fastmcp-server-template/pull/673)). Before starting it, set `MARKDOWN_VAULT_MCP_SOURCE_DIR` in `.env` to the host directory containing the vault. An existing proxy deployment should move its labels and shared network into `compose.override.yml`; remove the published port there with `ports: !reset []`. The override syntax needs Compose 2.24.4 or newer. Keep `MARKDOWN_VAULT_MCP_BASE_URL` set to the public URL when a proxy or OIDC is in use. The [Docker guide](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/deployment/docker/#behind-a-reverse-proxy) has the full overlay and explains how to select a locally built image ([template#577](https://github.com/pvliesdonk/fastmcp-server-template/pull/577), [#1465](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1465)).

The new [security model](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/security-model/index.md) page states what the server protects and what it leaves to the operator. Authentication is the access boundary, and the bind address only limits which machines can connect. The page also lists what the tools reach with the server's own privileges, and which routes answer without an MCP credential ([template#674](https://github.com/pvliesdonk/fastmcp-server-template/pull/674), [#1614](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1614)).
...
The four tools `fastmcp-pvl-core` registers on every server carry rewritten text for the model that reads it: `create_download_link` and `create_upload_link` now describe their `ref` and `ttl_s` parameters, which had no description before, `get_job_result` describes `job_id`, and `get_server_info` says when to call it. Names, schemas and behaviour are unchanged ([fastmcp-pvl-core#358](https://github.com/pvliesdonk/fastmcp-pvl-core/issues/358), [fastmcp-pvl-core#360](https://github.com/pvliesdonk/fastmcp-pvl-core/pull/360)).
...
A vault directory that does not exist is reported by name rather than as a crash. In 4.2 the server refused to start when `MARKDOWN_VAULT_MCP_SOURCE_DIR` was unset, and a value naming a missing directory passed validation, then "crashed in its lifespan with a raw `FileNotFoundError`" from the file watcher ([#1590](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1590)). An unset variable now means the container's mount point `/data/vault`. When the directory does not exist, the server still starts, so its tool listing and instructions are reachable, logs `vault_directory_missing` at ERROR, and every tool and link-minting call fails with `vault directory <path> does not exist. Set MARKDOWN_VAULT_MCP_SOURCE_DIR to the path of your markdown vault.` until the deployment is fixed and restarted. The `index`, `search` and `reindex` commands report the same message and exit 1. Managed Git mode is exempt because its bootstrap clones into the directory ([#1591](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1591), [#1581](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1581)). The [configuration reference](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/configuration/index.md) describes the variable.
...
A line-range `edit` with an empty `new_text` now deletes the lines. It used to leave one blank line in their place, and no edit mode could remove whole lines ([#1607](https://github.com/pvliesdonk/markdown-vault-mcp/issues/1607), [#1613](https://github.com/pvliesdonk/markdown-vault-mcp/pull/1613)).
...
1. If you use Compose, set `MARKDOWN_VAULT_MCP_SOURCE_DIR` in `.env`. Move proxy labels and networks to an override. Without a proxy, the server is published on `127.0.0.1:8000`; to reach it from other machines, configure authentication and publish on every interface from the override. If the MCP mount path changes, update the health check path too.
...
1. If a deployment relied on the server refusing to start when `MARKDOWN_VAULT_MCP_SOURCE_DIR` was unset, note that it now starts against `/data/vault` and reports a missing directory through `vault_directory_missing` in the log and through every tool call.
