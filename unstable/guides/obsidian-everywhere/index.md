...
1. Push your existing vault, or initialize a new vault and commit it
1. Confirm you can clone and pull from another machine
1. Add a `.gitignore` suitable for Obsidian workspace state (see Step 2)

## Step 2: Obsidian Desktop with obsidian-git

1. Install the `obsidian-git` plugin in Obsidian Desktop
1. Configure auto-pull and auto-push intervals
1. Set commit message patterns you can recognize later
1. Ensure workspace-local files are ignored (example):
...
1. Add an SSH key to your GitHub account (or app-specific auth)
1. Clone the same private vault repository to mobile
1. Configure periodic sync and test a note edit round trip

## Step 4: Server with markdown-vault-mcp

Follow [Docker](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/docker/index.md) for deployment details. For this topology, enable git write and server-side pull automation.
...
Use [OIDC Providers](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/oidc-providers/#authelia) for provider-specific setup, then [Docker OIDC setup](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/docker/#step-4-add-oidc-authentication) for container wiring.
...
Use [Claude Desktop](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/claude-desktop/index.md) to configure the MCP endpoint and verify tools. If you want semantic retrieval quality beyond keyword search, add embeddings via [Embeddings](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/embeddings/index.md).

Verification checklist:

1. `search` returns recent notes from git-synced vault content
1. `write` creates a new note
1. Server commits and pushes that note to the repository
1. Desktop/mobile clients pull and see the same note

### Firing prompts from Claude.ai's `+` menu

When Claude.ai is part of your setup, every MCP prompt this server exposes can be fired from the compose area's `+` menu once the server is added as a connector. Click `+`, select **connectors**, pick the server, pick a prompt; Claude opens with the invocation scaffolded. This is the recommended way to invoke multi-step prompts like `propose-links` or the PARA / Zettelkasten workflow prompts. See [How to invoke prompts](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/prompts/#how-to-invoke-prompts) for the full invocation reference.
...
- Add semantic search for better recall: [Embeddings](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/embeddings/index.md)
- Tune git modes and policies: [Git Integration](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/guides/git-integration/index.md)
- Add note templates to standardize AI-generated notes via the `create_from_template` prompt (see [MCP Prompts](https://pvliesdonk.github.io/markdown-vault-mcp/unstable/prompts/#create_from_template))
