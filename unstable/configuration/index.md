...
The file watcher never watches the directories that hold `INDEX_PATH`, `EMBEDDINGS_PATH`, and `STATE_PATH` (or `.git`), so the writes the server makes while indexing do not trigger it again. If you place one of these paths inside a content directory, that whole top-level directory stops being watched, so keep them at the vault root or outside the vault to keep sibling content watched live.
