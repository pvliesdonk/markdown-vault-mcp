...
1. **MCP Apps views** — browse your vault visually with Context Card, Graph Explorer, and more
...
- **OpenAI** — cloud-based, requires API key (see [Embeddings guide](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/embeddings/index.md))
...
For full details on views, configuration, and architecture, see the [MCP Apps guide](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/mcp-apps/index.md).

### Firing prompts from Claude.ai's `+` menu

This guide covers Claude Desktop. If you also use Claude.ai with the same server, its prompts can be fired from the compose area's `+` menu once the server is added as a connector. Click `+`, select **connectors**, pick the server, pick a prompt — Claude opens with the prompt scaffolded. See [How to invoke prompts](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/prompts/#how-to-invoke-prompts).
