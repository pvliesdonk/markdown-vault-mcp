...
1. **Read and extract the central intent.** Call `read(path=note)` and identify the central subject — is it a concrete deliverable, an ongoing responsibility, or reference material?
1. **Classify** into one of three buckets:
1. **Project** — concrete outcome plus a plausible deadline; something that can be completed.
1. **Area** — ongoing responsibility with no end state (e.g., "Health", "Team Management").
1. **Resource** — reference material, not actionable.
...
1. **Apply on confirmation.** On confirmation, `write` the note back with the `## Related` section added (robust to trailing whitespace), or use `edit` if you prefer to avoid rewriting the full file and can match the section heading precisely.
...
1. **Stale scan.** Compute a threshold of `now - 14 * 86400` seconds and flag any active project whose `modified_at` is older. These are the stale projects that need attention.
...
- **Read the design document** for details on the linking system and search algorithms: [`docs/design.md`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/design/index.md)
- **Explore the MCP tools** to understand the full API: [`tools/index.md`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/tools/index.md)
- **Review the examples** for templates and prompts: [`examples/para/`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/examples/para/index.md)
- **Prefer idea-centric knowledge management?** See the alternative workflow: [`docs/guides/zettelkasten.md`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/zettelkasten/index.md)
- **Ambient patterns**: [`docs/prompts.md`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/prompts/#ambient-patterns-without-prompts) — flows the LLM handles from prose alone (URL capture, research, ad-hoc link proposal)
- **Research workflows**: [research-workflows.md](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/research-workflows/index.md) — literature grounding, fact-checking, and writing papers from notes
