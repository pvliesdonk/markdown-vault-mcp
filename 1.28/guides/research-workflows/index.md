...
This guide is organising-scheme-agnostic. It works with [Zettelkasten](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/zettelkasten/index.md), [PARA](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/para/index.md), or no opinionated structure at all. The phase model in this guide composes the vault's generic search, linking, and write tools; it doesn't assume a particular folder layout.

## Setup

Install and configure markdown-vault-mcp per the [Claude Desktop guide](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/claude-desktop/index.md) or [Obsidian Everywhere](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/obsidian-everywhere/index.md) for the full multi-device setup. This guide assumes your vault is already connected to Claude and that you can invoke vault tools (`search`, `read`, `write`, `get_similar`, `get_context`) from a conversation.
...
- **From a conversation.** If your client exposes chat history (Claude.ai has `conversation_search` and `recent_chats`), the [`para-capture-chats`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/para/#para-capture-chats) prompt distils recent chats into Inbox-shaped notes ready for later triage. The vault doesn't need to be a PARA vault — the prompt produces plain notes with titles and bodies; you choose where they land.
- **From a research question.** The [`research`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/prompts/#research) builtin prompt takes a topic and produces a single synthesis note with a topic slug. Good for "I want to know what's out there on X" before you have a thesis.
...
A final point on frontmatter: keep it minimal at the seeding stage. A seed doesn't need a `type` field, a `status` field, or a rigid tag taxonomy. It needs a title, a `draft` tag, and a `created` date. If your vault follows a scheme like [Zettelkasten](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/zettelkasten/index.md) or [PARA](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/para/index.md), the triage step — not the seed step — is where a note acquires its typed frontmatter.
...
- [Zettelkasten](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/zettelkasten/index.md) — idea-centric organisation; pairs well with literature notes and atomic permanent notes.
- [PARA](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/para/index.md) — action-oriented organisation; research fits under Resources (reference material) and Projects (active research outputs).
- [Obsidian Everywhere](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/obsidian-everywhere/index.md) — multi-device setup for phone-first capture between sessions.
- [MCP Prompts reference](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/prompts/index.md) — especially the [ambient patterns](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/prompts/#ambient-patterns-without-prompts) and [`propose-links`](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/prompts/#propose-links).
