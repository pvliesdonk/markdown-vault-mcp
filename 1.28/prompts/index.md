...
1. LLM judgment: read each candidate's title and opening to confirm the connection is meaningful, not merely lexical.
1. Pick direction (one-way `A → B`) and placement per note shape (inline citation, `## Related` section, hub bullet list, footnote — whichever fits).
1. Show a batch preview of every proposed edit.
1. Write approved edits; skip failures (e.g., `ConcurrentModificationError`) and report reasons.
...
`propose-links` falls back to keyword search without embeddings, but the quality of candidates is noticeably better when `get_similar` is available. See [Embeddings](https://pvliesdonk.github.io/markdown-vault-mcp/1.28/guides/embeddings/index.md) for setup.
