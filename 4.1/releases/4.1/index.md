...
This also reached further than the one-shot migration tools. With `MARKDOWN_VAULT_MCP_OKF_WRITE` on, the convention layer rewrites `log.md` after every content write, and that path both failed to add frontmatter and stripped frontmatter an operator had added by hand. See the [OKF guide](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/guides/okf/index.md).
