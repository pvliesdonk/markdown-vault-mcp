...
Setting `GITHUB_WEBHOOK_SECRET` disables the filesystem watcher, the same way `GIT_PULL_INTERVAL_S > 0` does. Git rewrites the working tree during a checkout, and a watcher firing mid-checkout would scan a partial tree. Reindexing stays driven by the webhook and the periodic loop. See [File Watcher](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/configuration/#file-watcher).

The variable itself is listed in the [configuration reference](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/configuration/#github-webhook-push-triggered-pull).
...
The full enumeration of `pull.reason` and `push.reason` values lives in the [`git_sync` tool reference](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/tools/#git_sync).
