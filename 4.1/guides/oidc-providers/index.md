...
This guide covers configuring markdown-vault-mcp with specific OIDC providers. For general OIDC setup and architecture, see [OIDC Authentication](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/deployment/oidc/index.md).
...
For architecture details and a full Docker Compose deployment, see [OIDC Authentication](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/deployment/oidc/index.md).
...
Or in Docker, see [Docker OIDC setup](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/guides/docker/#step-4-add-oidc-authentication).
...
- **Session drops after hours**: Check that the `mcp_long_lived` lifespan is configured and referenced by the client. See the [token lifetime troubleshooting](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/guides/authentication/#session-drops-after-token-expiry).
...
If your reverse proxy already handles Google OIDC authentication, you can use **remote mode** instead: set only `BASE_URL` and `OIDC_CONFIG_URL`, omitting client credentials. See [Authentication: Remote mode](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/guides/authentication/#remote-mode).
