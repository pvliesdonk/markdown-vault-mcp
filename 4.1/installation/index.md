...
The `latest` tag is the newest stable release. For early adopters who want to test unreleased changes, the rolling `edge` tag tracks every merge to `main` and carries no version identity; see [Image tags](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/deployment/docker/#image-tags) for the full list. The floating `:latest`, `:vN`, and `:vN.M` tags only move on stable releases.

```
docker pull ghcr.io/pvliesdonk/markdown-vault-mcp:edge
```

See [Docker deployment](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/deployment/docker/index.md) for compose setup and volume configuration.
...
See the [systemd deployment guide](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/deployment/systemd/index.md) for full configuration and troubleshooting.
...
See the [Claude Code plugin guide](https://pvliesdonk.github.io/markdown-vault-mcp/4.1/guides/claude-code-plugin/index.md) for configuration and usage details.
...
- **Package root minimized (issue #903): import from submodules, not the package root.** The `markdown_vault_mcp` root package no longer re-exports the public API. Update library imports to their submodules:
...
Types such as `GroupedResult` come from `markdown_vault_mcp.types`. Only `__version__` remains importable from the root.
...
`MARKDOWN_VAULT_MCP_CHUNKS_PER_FILE` replaces `MARKDOWN_VAULT_MCP_CHUNKS_PER_DOC`. `SimilarItem` is removed; use `GroupedResult` (from `markdown_vault_mcp.types`).

- **Search returns snippets by default.** The `content` field carries a query-relevant snippet (approximately 200 words). Pass `snippet_words=0` to recover the prior full-chunk behaviour, or use `read(path, section=heading)` to fetch the full section after seeing a snippet.
...
- `MARKDOWN_VAULT_MCP_MAX_NOTE_READ_BYTES` caps whole-document `.md` reads (default 256 KB); reads above it raise `ValueError`. Partial reads via `read(path, section=heading)` bypass the cap.
