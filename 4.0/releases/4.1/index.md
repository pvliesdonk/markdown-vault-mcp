...
This is the first release since [4.0](https://pvliesdonk.github.io/markdown-vault-mcp/4.0/releases/4.0/index.md). Its through-line is defaults: several places where the server knew enough to do the right thing but waited to be told. It also brings two contributor features current. A size signal helps with planning, while a correctness pass covers the OKF reserved files 4.0 introduced.
...
This also reached further than the one-shot migration tools. With `MARKDOWN_VAULT_MCP_OKF_WRITE` on, the convention layer rewrites `log.md` after every content write, and that path both failed to add frontmatter and stripped frontmatter an operator had added by hand. See the [OKF guide](https://pvliesdonk.github.io/markdown-vault-mcp/4.0/guides/okf/index.md).
