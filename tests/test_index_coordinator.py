...
def test_changing_the_attachment_allowlist_rejects_the_warm_restart(
    ...
    an unchanged ``[[data.csv]]`` would keep the ``data.csv.md`` row it was
    built with after ``csv`` joined the allowlist (#1333).
    """
    db = tmp_path / "index.db"
    (tmp_path / "a.md").write_text("# A\n\n[[data.csv]]\n", encoding="utf-8")
...
def test_an_explicitly_empty_allowlist_is_not_the_default(tmp_path: Path) -> None:
    """Default set and explicit empty list derive different rows (#1333)."""
    db = tmp_path / "index.db"
    coord = make_coordinator(tmp_path, db=db)
    try:
        coord.build_index()
    ...
    coord2 = make_coordinator(tmp_path, db=db, attachment_extensions=())
    try:
        assert coord2._chunking_meta_matches() is False
    finally:
        coord2.close(timeout=5)
