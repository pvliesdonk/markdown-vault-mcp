...
The [`examples/okf/`](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/examples/okf/index.md) directory ships a declaration index, typed note templates (`Concept`, `Capture`), and a prompt pack (author a concept, verify a note, triage stale content, migrate a vault) that you can copy into your vault and prompt folders.
...
You can filter on these dimensions. `search` and `list_documents` accept `status`, `stale`, `trust_tier`, and `type` filters, so `{"stale": "true"}` or `{"status": "deprecated"}` builds a triage listing. See the [tools reference](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/tools/index.md) for the full filter set.
...
OKF composes with the [PARA](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/guides/para/index.md) and [Zettelkasten](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/guides/zettelkasten/index.md) methods. They organise where notes live and how work flows; OKF describes what each note is. A PARA or Zettelkasten vault can also be an OKF bundle. Three points of overlap matter (each method's guide has a matching section from its own angle: [Using PARA with OKF](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/guides/para/#using-para-with-okf), [Using Zettelkasten with OKF](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/guides/zettelkasten/#using-zettelkasten-with-okf)):
