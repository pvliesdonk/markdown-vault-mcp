...
- **Folder conventions**: per-folder `_conventions.md` files carry your authoring rules, surfaced to LLM clients at write time via [`get_conventions`](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/tools/#get_conventions) and in `write`/`edit` results
- **[Open Knowledge Format](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/guides/okf/index.md)**: recognizes OKF bundles (an `okf_version` declaration in the root `index.md`) and annotates search/read results with each note's type, lifecycle status, staleness, and trust tier; those dimensions are filterable and nudge ranking, and the server ships an `okf_validate` audit, one-shot migration transforms, an optional enforced write layer (provenance + verification + `log.md`/`index.md` upkeep), and a downloadable bundle export
...
- **[Configuration Generator](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/configuration-generator/index.md)**: build a working config, Docker command, or systemd unit in your browser.
...
- **"Research and create a set of interlinked notes."** Claude composes web tools + `write` with wikilinks. See the [Research workflows guide](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/guides/research-workflows/index.md) for the full loop.
- **"Summarize today's conversations into Inbox notes."** Claude.ai composes `conversation_search` + `recent_chats` + `write`; the [`para-capture-chats`](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/guides/para/#using-the-para-prompts) prompt is the one-click version.
- **Find missing links.** The [`propose-links`](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/prompts/index.md) builtin prompt scans recently modified notes and proposes useful connections.

See [MCP Prompts](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/prompts/index.md) for the codified workflows and the ambient-pattern reference.

## Release notes

Per-minor [release notes](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/releases/index.md) explain what changed in each release, who is affected, and what to check before upgrading. The GitHub release body links to the matching page.
...
See [Installation](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/installation/index.md) for all installation methods (PyPI, uv, Docker, Linux packages, Claude Code plugin) and [Configuration](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/configuration/index.md) for all available options.
