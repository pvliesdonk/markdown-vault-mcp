...
`conventions_file` is the configured per-folder conventions filename (`null` when the feature is disabled); `convention_folders` lists every folder carrying such a file (`""` is the vault root). See the [`get_conventions` tool](https://pvliesdonk.github.io/markdown-vault-mcp/5.0/tools/#get_conventions). `exclude_patterns` reports the vault's *effective* exclusions: the configured patterns plus the two patterns derived from `conventions_file` (shown in the example above), so the list always matches actual index behavior.
