...
| `MARKDOWN_VAULT_MCP_OKF_MODE` | `auto` | No | OKF (Open Knowledge Format) read semantics: auto follows the vault's okf_version declaration in the root index.md, off never applies OKF semantics, on forces them without a declaration. Read-only annotations either way; OKF never changes write behavior in this mode. |
