...
| `MARKDOWN_VAULT_MCP_SEARCHABLE_FIELDS` | none | No | Comma-separated frontmatter fields whose text values become keyword-searchable, and also enrich first-chunk embedding input (independent of `EMBED_CONTEXT`). Setting it cold-rebuilds the index and re-embeds the vault once on next startup |
